# CJDNS support in Aixcoin Core

It is possible to run Aixcoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Aixcoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Aixcoin network and individual nodes.
...
before it will work with your Aixcoin Core node. This is described in steps
...
## Run Aixcoin Core with CJDNS

Once you are connected to the CJDNS network, the following Aixcoin Core
...
When enabled, this option tells Aixcoin Core that it is running in an
...
IPv6 local network. This helps Aixcoin Core perform better address management:
...
CJDNS support was added to Aixcoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `aixcoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/aixcoin/aixcoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Aixcoin Core

There are several ways to see your CJDNS address in Aixcoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `aixcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `aixcoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `aixcoin-cli help getnodeaddresses` for details.
