Aixcoin Core version 28.0 is now available from:

  <https://aixcoin-core.github.io/aixcoincore.org/bin/aixcoin-core-28.0/>
...
Running Aixcoin Core binaries on macOS requires self signing.
```
cd /path/to/aixcoin-28.0/bin
...
Support for Testnet4 as specified in [BIP94](https://github.com/aixcoin/bips/blob/master/bip-0094.mediawiki)
...
The default data directory on Windows has been moved from `C:\Users\Username\AppData\Roaming\Aixcoin`
to `C:\Users\Username\AppData\Local\Aixcoin`. Aixcoin Core will check the existence
...
See [JSON-RPC-interface.md](https://github.com/aixcoin/aixcoin/blob/master/doc/JSON-RPC-interface.md#json-rpc-11-vs-20) for details. (#27101)
...
libaixcoinconsensus Removal
---------------------------

The libaixcoin-consensus library was deprecated in 27.0 and is now completely removed. (#29648)

P2P and Network Changes
-----------------------

- Previously if Aixcoin Core was listening for P2P connections, either using
...
- Aixcoin Core will now fail to start up if any of its P2P binds fail, rather
...
- Transactions with version number set to 3 are now treated as standard on all networks (#29496),
  ...
  described in [BIP 431](https://github.com/aixcoin/bips/blob/master/bip-0431.mediawiki).  The
...
are now required to compile Aixcoin Core. (#29091, #30263)

- The minimum required glibc to run Aixcoin Core is now
...
- A new `-testdatadir` option has been added to `test_aixcoin` to allow specifying the
