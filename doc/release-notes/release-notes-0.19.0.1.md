Aixcoin Core version 0.19.0.1 is now available from:

  <https://aixcoin-core.github.io/aixcoincore.org/bin/aixcoin-core-0.19.0.1/>
...
Aixcoin Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use Aixcoin Core on unsupported systems.
...
imposed by Apple, which the Aixcoin Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/aixcoin/aixcoin/blob/master/doc/reduce-memory.md)
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `aixcoind -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile Aixcoin Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in Aixcoin Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of Aixcoin Core. (#14054)

- To eliminate well-known denial-of-service vectors in Aixcoin Core,
...
- By default, Aixcoin Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `aixcoin-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how Aixcoin Core determines
...
- If a Tor hidden service is being used, Aixcoin Core will be bound to
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  AIX), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in AIX per 1,000
...
- #15649 Add ChaCha20Poly1305@Aixcoin AEAD (jonasschnelli)
...
- #15639 aixcoin-wallet tool: Drop libaixcoin_server.a dependency (ryanofsky)
...
- #16521 Use the default maxfeerate value as AIX/kB (Remagpie)
...
- #16366 init: Use InitError for all errors in aixcoind/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "aixcoin-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of aixcoind build (NicolasDorier)
...
- #16441 remove qt libjpeg check from aixcoin_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_aixcoin to auto generated project files (sipsorcery)
...
- #15773 Add AixcoinTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_aixcoin output (practicalswift)
...
- #15896 feature_filelock, interface_aixcoin_cli: Use PACKAGE_NAME in messages rather than hardcoding Aixcoin Core (luke-jr)
...
- #16277 Suppress output in test_aixcoin for expected errors (gertjaap)
...
- #16929 follow-up to rpc: default maxfeerate value as AIX/kB (jonatack)
...
- #16489 log: harmonize aixcoind logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (aixcoinVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example aixcoin.conf (fanquake)
...
- #15683 Comment for seemingly duplicate LIBAIXCOIN_SERVER (Bushstar)
...
- #16080 build/doc: update aixcoin_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in aixcoin.conf (fanquake)
...
- #16621 add default aixcoin.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the aixcoin-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, aixcoin.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/aixcoind.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/aixcoin/aixcoin/).
