...
Aixcoin Core version 0.20.0 is now available from:
  ...
  <https://aixcoin-core.github.io/bin/aixcoin-core-0.20.0/>
...
From Aixcoin Core 0.20.0 onwards, macOS versions earlier than 10.12 are no
...
- Instead of running `make` simply, you should instead run
  `AIXCOIN_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Aixcoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Aixcoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Aixcoin P2P network protocol
...
- OpenSSL is no longer used by Aixcoin Core.  (#17265)

- BIP70 support has been fully removed from Aixcoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/aixcoin/aixcoin/issues/16599),
  [PR #16702](https://github.com/aixcoin/aixcoin/pull/16702), and the
  `aixcoind help` for more information.  This option is experimental and
...
- All custom settings configured when Aixcoin Core starts are now
...
- The "Start Aixcoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://aixcoin-core.github.io/en/releases/0.18.0/#wallet-gui)
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  program such as [HWI](https://github.com/aixcoin-core/HWI) for
...
- Aixcoin Core's automatically-generated source code documentation is
  now available at https://doxygen.aixcoin-core.github.io.  (#17596)
...
- The `aixcoin-cli` utility used with the `-getinfo` parameter now
...
- The `aixcoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `aixcoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_aixcoin-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBAIXCOIN_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_aixcoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBAIXCOIN_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for aixcoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate aixcoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest aixcoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_aixcoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (aixcoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone aixcoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Aixcoin Core files (hebasto)
...
- #17119 Fix broken aixcoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from aixcoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.aixcoin-core.github.io (laanwj)
...
- #17648 Rename wallet-tool references to aixcoin-wallet (hel-o)
...
- #18341 Replace remaining literal AIX with `CURRENCY_UNIT` (domob1812)
...
- #16115 On aixcoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from aixcoin-cli -getinfo (malevolent)
