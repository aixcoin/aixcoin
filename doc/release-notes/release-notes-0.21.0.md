...
Aixcoin Core version 0.21.0 is now available from:

  <https://aixcoin-core.github.io/aixcoincore.org/bin/aixcoin-core-0.21.0/>
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  [BIP155](https://github.com/aixcoin/bips/blob/master/bip-0155.mediawiki).
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. Aixcoin Core will no longer
...
- This release adds support for serving
  [BIP157](https://github.com/aixcoin/bips/blob/master/bip-0157.mediawiki) compact
...
- This release adds support for signets
  ([BIP325](https://github.com/aixcoin/bips/blob/master/bip-0325.mediawiki)) in
...
- This release implements
  [BIP339](https://github.com/aixcoin/bips/blob/master/bip-0339.mediawiki)
...
- This release implements the proposed Taproot consensus rules
  ([BIP341](https://github.com/aixcoin/bips/blob/master/bip-0341.mediawiki) and
  [BIP342](https://github.com/aixcoin/bips/blob/master/bip-0342.mediawiki)),
...
- A new `aixcoin-cli -netinfo` command provides a network peer connections
...
- A new `aixcoin-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `aixcoin-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `aixcoin-cli -getinfo` is now expanded to return a JSON
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `aixcoin-wallet` tool which performs the salvage
...
Aixcoin Core will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`aixcoin.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
Aixcoin Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `aixcoin-cli createwallet` or `aixcoin-wallet
...
Bugs and missing functionality can be reported to the [issue tracker](https://github.com/aixcoin/aixcoin/issues).
...
behave as they did in previous versions of Aixcoin Core.
...
The change to using descriptors changes the default derivation paths used by Aixcoin Core
...
- A new `fee_rate` parameter/option denominated in satoshis per vbyte (sat/vB)
  ...
  `walletcreatefundedpsbt` still exists for setting a fee rate in AIX per 1,000
...
- The `bumpfee` RPC `fee_rate` option is changed from AIX/kvB to sat/vB and the
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  ...
  `\<datadir\>/settings.json` and augments any command line or `aixcoin.conf`
...
- #19284 Add seed.aixcoin.wiz.biz to DNS seeds (wiz)
...
- #20198 Show name, format and if uses descriptors in aixcoin-wallet tool (jonasschnelli)
...
- #19133 rpc, cli, test: add aixcoin-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in aixcoin-cli (laanwj)
...
### Build system
- #18504 Drop aixcoin-tx and aixcoin-wallet dependencies on libevent (ryanofsky)
...
- #18297 Use pkg-config in AIXCOIN_QT_CONFIGURE for all hosts including Windows (hebasto)
...
- #18653 add coverage for aixcoin-cli -rpcwait (jonatack)
...
- #18739 Document how to fuzz Aixcoin Core using Honggfuzz (practicalswift)
...
- #19061 Add link to Visual Studio build readme (maitreaixcoin)
...
- #18817 Document differences in aixcoind and aixcoin-qt locale handling (practicalswift)
