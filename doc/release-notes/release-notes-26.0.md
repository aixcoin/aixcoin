...
Aixcoin Core version 26.0 is now available from:

  <https://aixcoin-core.github.io/aixcoincore.org/bin/aixcoin-core-26.0/>
...
- Experimental support for the v2 transport protocol defined in
  [BIP324](https://github.com/aixcoin/bips/blob/master/bip-0324.mediawiki) was added.
...
- [Miniscript](https://aixcoin.sipa.be/miniscript/) expressions can now be used in Taproot descriptors for all RPCs working with descriptors. (#27255)

- `finalizepsbt` is now able to finalize a PSBT with inputs spending [Miniscript](https://aixcoin.sipa.be/miniscript/)-compatible Taproot leaves. (#27255)
...
- `aixcoind` and `aixcoin-qt` will now raise an error on startup
 ...
 the aixcoin.conf contained in it. (#27302)
...
- A new `aixcoinconsensus_verify_script_with_spent_outputs` function is available in libconsensus which optionally accepts the spent outputs of the transaction being verified.
- A new `aixcoinconsensus_SCRIPT_FLAGS_VERIFY_TAPROOT` flag is available in libconsensus that will verify scripts with the Taproot spending rules.
...
```sh
src/aixcoin-cli -named bumpfee txid fee_rate=100
```

instead of

```sh
src/aixcoin-cli -named bumpfee txid options='{"fee_rate": 100}'
```
...
- It's now possible to use [Miniscript](https://aixcoin.sipa.be/miniscript/) inside Taproot leaves for descriptor wallets. (#27255)
...
- Bash completion files have been renamed from `aixcoin*.bash-completion` to
  `aixcoin*.bash`. This means completions can be automatically loaded on demand
