Aixcoin Core version 0.18.0 is now available from:

  <https://aixcoin-core.github.io/aixcoincore.org/bin/aixcoin-core-0.18.0/>
...
`/Applications/Aixcoin-Qt` (on Mac) or `aixcoind`/`aixcoin-qt` (on
Linux).
...
recommended to use Aixcoin Core on unsupported systems.

Aixcoin Core should also work on most other Unix-like systems but is not
...
`contrib/init/aixcoind.service`, it has been changed to use
`/var/lib/aixcoind` as the data directory instead of
`~aixcoin/.aixcoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/aixcoind` will
exist has enough space (check using `df -h /var/lib/aixcoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/aixcoin/aixcoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://aixcoin-core.github.io/aixcoincore.org/en/contact/) the Aixcoin Core
...
The systemd init file (`contrib/init/aixcoind.service`) has been changed
to use `/var/lib/aixcoind` as the data directory instead of
`~aixcoin/.aixcoin`. This change makes Aixcoin Core more consistent with
...
`datadir` option in `/etc/aixcoin/aixcoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/aixcoin/aixcoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/aixcoin/aixcoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/aixcoin/aixcoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/aixcoin/aixcoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/aixcoin/aixcoin/blob/master/doc/aixcoin-conf.md)
  about the `aixcoin.conf` file describes how to use it to configure
  Aixcoin Core.

- A new document introduces Aixcoin Core's BIP174 [Partially-Signed
  Aixcoin Transactions
  (PSBT)](https://github.com/aixcoin/aixcoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/aixcoin/aixcoin/blob/master/doc/descriptors.md)
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/aixcoin/aixcoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/aixcoin/aixcoin/blob/master/doc/descriptors.md).
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/aixcoin/aixcoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/aixcoin/aixcoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart aixcoind with the `-deprecatedrpc=generate`
...
- A new `aixcoin-wallet` tool is now distributed alongside Aixcoin
...
This section describes planned changes to Aixcoin Core that may affect
other Aixcoin software and services.

- Since version 0.16.0, Aixcoin Core’s built-in wallet has defaulted to
  ...
  set the `addresstype=bech32` configuration option in any Aixcoin Core
...
- A sub-project of Aixcoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Aixcoin Core.  See their [project
  page](https://github.com/aixcoin-core/HWI#readme) for details.
...
- On macOS, Aixcoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `aixcoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libaixcoin_server.a` (jnewbery)
...
- #13926 [Tools] aixcoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like aixcoind (MarcoFalke)
...
- #14521 qt, docs: Fix `aixcoin-qt -version` output formatting (hebasto)
...
- #11625 Add AixcoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libaixcoin_server` (laanwj)
...
- #15154 Configure: aixcoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/aixcoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Aixcoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libaixcoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in aixcoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Aixcoin Core (Christewart)
...
- #12246 Bugfix: Only run aixcoin-tx tests when aixcoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_aixcoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_aixcoin` (MarcoFalke)
...
- #14186 aixcoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_aixcoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update aixcoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/aixcoin-conf.md (hebasto)
...
- #15354 Add missing `aixcoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct aixcoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support aixcoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/aixcoin/).
