...
Below are some notes on how to build Aixcoin Core for Windows.

The options known to work for building Aixcoin Core on Windows are:
...
Note that for WSL the Aixcoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/aixcoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\aixcoin`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/aixcoin
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/aixcoin --strip
```
