# PSBT Howto for Aixcoin Core

Since Aixcoin Core 0.17, an RPC interface exists for Partially Signed Aixcoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/aixcoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Aixcoin transactions that are not fully signed
...
#### Multisig with multiple Aixcoin Core instances
...
Aixcoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `aixcoin-cli` is used.
...
Later, when *V* AIX has been received on *Amulti*, and Bob and Carol want to
