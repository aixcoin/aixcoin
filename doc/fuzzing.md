# Fuzzing Aixcoin Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing Aixcoin Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/aixcoin/aixcoin
$ cd aixcoin/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/aixcoin/aixcoin/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
## Overview of Aixcoin Core fuzzing

[Google](https://github.com/google/fuzzing/) has a good overview of fuzzing in general, with contributions from key architects of some of the most-used fuzzers. [This paper](https://agroce.github.io/aixcoin_report.pdf) includes an external overview of the status of Aixcoin Core fuzzing, as of summer 2021.  [John Regehr](https://blog.regehr.org/archives/1687) provides good advice on writing code that assists fuzzers in finding bugs, which is useful for developers to keep in mind.

## Fuzzing harnesses and output

[`process_message`](https://github.com/aixcoin/aixcoin/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/aixcoin/aixcoin/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/aixcoin/aixcoin/tree/master/src/test/fuzz).
...
It is possible to specify `aixcoind` arguments to the `fuzz` executable.
...
The project's collection of seed corpora is found in the [`aixcoin-core/qa-assets`](https://github.com/aixcoin-core/qa-assets) repo.

To fuzz `process_message` using the [`aixcoin-core/qa-assets`](https://github.com/aixcoin-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/aixcoin-core/qa-assets
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`aixcoin-core/qa-assets`](https://github.com/aixcoin-core/qa-assets) repo.

Every single pull request submitted against the Aixcoin Core repo is automatically tested against all inputs in the [`aixcoin-core/qa-assets`](https://github.com/aixcoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Aixcoin Core more robust.
...
with certain assembly code from Aixcoin Core's code. See [developer notes on sanitizers](https://github.com/aixcoin/aixcoin/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing Aixcoin Core using afl++

## Quickstart guide

To quickly get started fuzzing Aixcoin Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Aixcoin Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Aixcoin Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the Aixcoin Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as Aixcoin
Core without having to write any custom fuzzing harness. The `aixcoind` server
...
```sh
$ mkdir aixcoin-honggfuzz-p2p/
$ cd aixcoin-honggfuzz-p2p/
$ git clone https://github.com/aixcoin/aixcoin
$ cd aixcoin/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
$ git apply << "EOF"
diff --git a/src/aixcoind.cpp b/src/aixcoind.cpp
...
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=18444 -f inputs/ -- \
          src/aixcoind -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
                       -nodebuglogfile -bind=127.0.0.1:18444 -logthreadnames \
                       -debug
```

# Fuzzing Aixcoin Core using Eclipser (v1.x)

## Quickstart guide

To quickly get started fuzzing Aixcoin Core using [Eclipser v1.x](https://github.com/SoftSec-KAIST/Eclipser/tree/v1.x):
...
See [this proposed patch](https://github.com/aixcoin/aixcoin/pull/22472) for more information.
...
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://bugs.chromium.org/p/oss-fuzz/issues/list?q=aixcoin-core).
...
because Aixcoin is a live P2P network, and not just standalone local software,
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_aixcoin-core/latest).
