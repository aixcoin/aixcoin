...
export TEST_RUNNER_EXTRA="--previous-releases --coverage --extended --exclude feature_dbcrash --exclude rpc_openrpc"  # Run extended tests so that coverage does not fail, but exclude the very slow dbcrash and rpc_openrpc (Debug builds add the "lock" log category, changing the logging RPC description)
export GOAL="install"
