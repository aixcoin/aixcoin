...
class WalletAncientMigrationTest(BitcoinTestFramework):
    ...
    def dumb_sync_blocks(self):
        """Sync blocks from old node to modern node via RPC (submitblock).

        Used because v0.14.3 lacks syncwithvalidationinterfacequeue, so the
        normal sync helpers cannot be used."""
        node_from = self.nodes[0]
        node_to = self.nodes[1]
        to_height = node_from.getblockcount()
        height = node_to.getblockcount()
        for i in range(height + 1, to_height + 1):
            b = node_from.getblock(blockhash=node_from.getblockhash(i), verbose=False)
            node_to.submitblock(b)
        assert_equal(node_to.getblockcount(), to_height)
