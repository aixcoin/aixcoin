...
class FilelockTest(AixcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Check that we can't start a second aixcoind instance using the same datadir")
        ...
        self.log.info("Check that we can't start a second aixcoind instance using the same blocksdir")
        ...
        self.log.info("Check that cookie and PID file are not deleted when attempting to start a second aixcoind using the same datadir/blocksdir")
        cookie_file = datadir / ".cookie"
        assert cookie_file.exists()  # should not be deleted during the second aixcoind instance shutdown
        pid_file = datadir / AIXCOIN_PID_FILENAME_DEFAULT
        ...
        if self.is_wallet_compiled():
            ...
            self.log.info("Check that we can't start a second aixcoind instance using the same wallet")
