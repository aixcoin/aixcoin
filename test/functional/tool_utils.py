...
# Copyright 2016-present The Aixcoin Core developers
...
class ToolUtils(AixcoinTestFramework):
    ...
    def skip_test_if_missing_module(self):
        self.skip_if_no_aixcoin_tx()
        self.skip_if_no_aixcoin_util()

    def run_test(self):
        self.testcase_dir = Path(self.config["environment"]["SRCDIR"]) / "test" / "functional" / "data" / "util"
        self.bins = self.get_binaries()
        with open(self.testcase_dir / "aixcoin-util-test.json") as f:
