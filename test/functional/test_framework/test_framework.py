...
class AixcoinTestFramework(metaclass=AixcoinTestMetaClass):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop aixcoinds after the test execution")
    ...
    def setup(self):
        ...
        fname_aixcoind = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "aixcoind" + config["environment"]["EXEEXT"],
        )
        fname_aixcoincli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "aixcoin-cli" + config["environment"]["EXEEXT"],
        )
        fname_aixcoinutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: aixcoinds were not stopped and may still be running")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'aixcoind', self.options.aixcoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'aixcoin-cli', self.options.aixcoincli) for v in versions]
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    aixcoind=self.options.aixcoind,
                    aixcoin_cli=self.options.aixcoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
