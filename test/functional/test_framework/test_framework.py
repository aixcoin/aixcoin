...
TMPDIR_PREFIX = "aixcoin_func_test_"
...
class AixcoinTestMetaClass(type):
    """Metaclass for AixcoinTestFramework.

    Ensures that any attempt to register a subclass of `AixcoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'AixcoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("AixcoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("AixcoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class AixcoinTestFramework(metaclass=AixcoinTestMetaClass):
    """Base class for a aixcoin test script.

    Individual aixcoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave aixcoinds and test.* datadir on exit or error")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use aixcoin-cli instead of RPC for all commands")
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("AixcoinRPC")
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a aixcoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple aixcoind test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the aixcoind
    ...
    def _start_logging(self):
        ...
        # Format logs the same as aixcoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in aixcoin.conf
    ...
    def skip_if_no_aixcoind_tracepoints(self):
        """Skip the running test if aixcoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("aixcoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_aixcoind_zmq(self):
        """Skip the running test if aixcoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("aixcoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if aixcoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("aixcoin-wallet has not been compiled")

    def skip_if_no_aixcoin_tx(self):
        """Skip the running test if aixcoin-tx has not been compiled."""
        if not self.is_aixcoin_tx_compiled():
            raise SkipTest("aixcoin-tx has not been compiled")

    def skip_if_no_aixcoin_util(self):
        """Skip the running test if aixcoin-util has not been compiled."""
        if not self.is_aixcoin_util_compiled():
            raise SkipTest("aixcoin-util has not been compiled")

    def skip_if_no_aixcoin_chainstate(self):
        """Skip the running test if aixcoin-chainstate has not been compiled."""
        if not self.is_aixcoin_chainstate_compiled():
            raise SkipTest("aixcoin-chainstate has not been compiled")

    def skip_if_no_aixcoin_bench(self):
        """Skip the running test if bench_aixcoin has not been compiled."""
        if not self.is_bench_compiled():
            raise SkipTest("bench_aixcoin has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if aixcoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("aixcoin-cli has not been compiled.")
    ...
    def is_bench_compiled(self):
        """Checks whether bench_aixcoin was compiled."""
        return self.config["components"].getboolean("BUILD_BENCH")

    def is_cli_compiled(self):
        """Checks whether aixcoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether aixcoin-wallet was compiled."""
