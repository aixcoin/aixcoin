...
# TestP2PConn: A peer we use to send messages to aixcoind, and store responses.
...
class CompactBlocksTest(AixcoinTestFramework):
    ...
    # This test actually causes aixcoind to (reasonably!) disconnect us, so do this last.
    ...
    # Test that aixcoind requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for aixcoind to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        node = self.nodes[0]
        # aixcoind will not send blocktxn responses for blocks whose height is
