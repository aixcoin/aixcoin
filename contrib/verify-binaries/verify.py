...
"""Script for verifying Aixcoin Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from aixcoin-core.github.io and aixcoin.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/aixcoin-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://aixcoin-core.github.io"
HOST2 = "https://aixcoin.org"
VERSIONPREFIX = "aixcoin-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Aixcoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"aixcoin_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by aixcoin-core.github.io
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the aixcoin release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes aixcoin.org lags behind aixcoin-core.github.io.)')
    )
