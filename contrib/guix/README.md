# Bootstrappable Aixcoin Core Builds

This directory contains the files necessary to perform bootstrappable Aixcoin
Core builds.
...
To build Aixcoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Aixcoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/aixcoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/aixcoin-core/guix.sigs).
