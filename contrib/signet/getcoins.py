...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing aixcoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='aixcoin-cli', help='aixcoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Aixcoin address to which the faucet should send')
...
parser.add_argument('aixcoin_cli_args', nargs='*', help='Arguments to pass on to aixcoin-cli (default: -signet)')

args = parser.parse_args()

if args.aixcoin_cli_args == []:
    args.aixcoin_cli_args = ['-signet']


def aixcoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.aixcoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = aixcoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = aixcoin_cli(['getnewaddress', 'faucet', 'bech32'])
