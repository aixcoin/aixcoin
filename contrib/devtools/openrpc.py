...
def _arg_schema(arg: dict[str, Any]) -> dict[str, Any]:
    ...
    if t == "array":
        if len(inner) > 1:
            items = {"oneOf": [_arg_schema(i) for i in inner]}
        elif inner:
            items = _arg_schema(inner[0])
        else:
            items = {}
    ...
    if t == "object_user_keys":
        additional = _arg_schema(inner[0]) if inner else True
...
def _result_schema(result: dict[str, Any]) -> dict[str, Any]:
    ...
    if t == "array":
        if len(inner) > 1:
            items = {"oneOf": [_result_schema(i) for i in inner]}
        elif inner:
            items = _result_schema(inner[0])
        else:
            items = {}
        return {"type": "array", "items": items}
    if t == "array_fixed":
