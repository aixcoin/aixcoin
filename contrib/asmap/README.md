...
be used in Aixcoin Core with the `-asmap` option.
...
to binary before being used with Aixcoin Core.
...
the `aixcoin-cli getnodeaddresses` command.
...
```
aixcoin-cli getnodeaddresses 0 > addrs.json
```
