...
namespace init {
namespace {
const char* EXE_NAME = "aixcoind";

class AixcoindInit : public interfaces::Init
{
public:
    AixcoindInit(NodeContext& node) : m_node(node)
    {
        InitContext(m_node);
        m_node.init = this;
    }
...
{
    return std::make_unique<init::AixcoindInit>(node);
}
} // namespace interfaces
