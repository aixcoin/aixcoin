// Copyright (c) 2025-present The Aixcoin Core developers
...
Options:
  -m, --multiprocess     Run multiprocess binaries aixcoin-node, aixcoin-gui.
  -M, --monolithic       Run monolithic binaries aixcoind, aixcoin-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'aixcoin-qt [ARGS]' or 'aixcoin-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'aixcoind [ARGS]' or 'aixcoin-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'aixcoin-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'aixcoin-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'aixcoin-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_aixcoin [ARGS]'.
  chainstate [ARGS] Run aixcoin kernel chainstate util, equivalent to running 'aixcoin-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_aixcoin [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_aixcoin-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "aixcoin-gui" : "aixcoin-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "aixcoin-node" : "aixcoind");
        } else if (cmd.command == "rpc") {
            args.emplace_back("aixcoin-cli");
            // Since "aixcoin rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("aixcoin-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("aixcoin-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_aixcoin");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("aixcoin-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_aixcoin");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_aixcoin-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("aixcoin-util");
        } else {
...
//! Execute the specified aixcoind, aixcoin-qt or other command line in `args`
...
//!             be a relative path to a aixcoind, aixcoin-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/aixcoin ...
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // (https://github.com/aixcoin/aixcoin/pull/31375#discussion_r1861814807)
