...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking aixcoin code so they have to run
    ...
    // functions run, so client objects are fully disconnected before aixcoin
    // code in the destructors are run. This way if the aixcoin code tries to
...
void EventLoop::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In aixcoin core, running these callbacks in the right order is
