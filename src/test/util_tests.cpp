...
{
    util::TokenBucket<NodeClock> b(/*rate=*/1, /*value=*/2, /*cap=*/10, /*debt=*/-3);
    BOOST_CHECK(b.decrement(1));  // 2 -> 1
    BOOST_CHECK(b.decrement(1));  // 1 -> 0
    BOOST_CHECK(b.decrement(1));  // 0 -> -1, still above -3
    BOOST_CHECK(b.decrement(1));  // -1 -> -2, still above -3
    BOOST_CHECK(!b.decrement(1)); // -2 -> -3, at floor
