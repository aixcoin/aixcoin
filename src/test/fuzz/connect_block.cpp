...
#include <validation.cpp>  // Needed here to get complete definition of ConnectTrace
...
struct TestChainstate : public Chainstate {
    bool CallActivateBestChainStep(BlockValidationState& state, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace) EXCLUSIVE_LOCKS_REQUIRED(cs_main, m_mempool->cs)
    {
        return ActivateBestChainStep(state, pindexMostWork, pblock, fInvalidFound, connectTrace);
    }
...
static void loadCurrentBlock(Chainstate& CState, CBlockIndex* currentBlock) {
    ...
    if (allUTXOCTxIn.size() <= (size_t) currentBlock->nHeight) {
        allUTXOCTxIn.resize(currentBlock->nHeight + 1);
    }
    ...
    for (const auto& tx: listBlocks[currentBlock->nHeight]->vtx) {
        for (unsigned voutIndex = 0; voutIndex < tx->vout.size(); voutIndex++) {
            ...
            allUTXOCTxIn[currentBlock->nHeight] = getSpendingScript(*tx, voutIndex);
        }
    }
}
...
FUZZ_TARGET(activate_best_chain_step, .init = initialize_connect_block) {
    ...
    do {
        ...
        if (!active_chainstate.CallActivateBestChainStep(state, currentTipIndex, currentBlock, foundInvalid, connectTrace)) {
...
FUZZ_TARGET(activate_best_chain, .init = initialize_connect_block) {
    ...
    LIMITED_WHILE(fuzzed_data_provider.ConsumeBool(), 3) {
        ...
        DEBUGOUTPUT(std::cout << "Step1 generate valid block: " << currentBlock->GetHash() << std::endl);
    ...
    LIMITED_WHILE(fuzzed_data_provider.ConsumeBool() || branch2.size() <= branch1.size(), 5) {
        ...
        DEBUGOUTPUT(std::cout << "Step2 generate valid block: " << currentBlock->GetHash() << std::endl);
        tmpIndex.emplace_back(std::make_unique<CBlockIndex>(*currentBlock));
        tmpIndex.back()->nHeight = currentTipIndex->nHeight + 1;
        tmpIndex.back()->pprev = currentTipIndex;
        currentTipIndex = tmpIndex.back().get();
