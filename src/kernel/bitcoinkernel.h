...
#ifndef AIXCOIN_KERNEL_AIXCOINKERNEL_H
#define AIXCOIN_KERNEL_AIXCOINKERNEL_H
...
#ifndef AIXCOINKERNEL_API
    #ifdef AIXCOINKERNEL_BUILD
        #if defined(_WIN32)
            #define AIXCOINKERNEL_API __declspec(dllexport)
        #else
            #define AIXCOINKERNEL_API __attribute__((visibility("default")))
        #endif
    #else
        #if defined(_WIN32) && !defined(AIXCOINKERNEL_STATIC)
            #define AIXCOINKERNEL_API __declspec(dllimport)
        #else
            #define AIXCOINKERNEL_API
        #endif
    #endif
#endif
...
/* Warning attributes */
#if defined(__GNUC__)
    #define AIXCOINKERNEL_WARN_UNUSED_RESULT __attribute__((__warn_unused_result__))
#else
    #define AIXCOINKERNEL_WARN_UNUSED_RESULT
#endif

/**
 * AIXCOINKERNEL_ARG_NONNULL is a compiler attribute used to indicate that
 ...
 */
#if !defined(AIXCOINKERNEL_BUILD) && defined(__GNUC__)
    #define AIXCOINKERNEL_ARG_NONNULL(...) __attribute__((__nonnull__(__VA_ARGS__)))
#else
    #define AIXCOINKERNEL_ARG_NONNULL(...)
#endif
 ...
 *
 * This header currently exposes an API for interacting with parts of Aixcoin
 ...
 * also not yet included in releases of Aixcoin Core.
 *
...
AIXCOINKERNEL_API AIXk_ChainstateManagerOptions* AIXCOINKERNEL_WARN_UNUSED_RESULT AIXk_chainstate_manager_options_create(
    ...
    size_t blocks_directory_len) AIXCOINKERNEL_ARG_NONNULL(1);
...
#endif // AIXCOIN_KERNEL_AIXCOINKERNEL_H
