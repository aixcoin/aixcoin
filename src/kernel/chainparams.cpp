...
public:
    CMainParams() {
        ...
        vSeeds.emplace_back("seed.aixcoin.sipa.be."); // Pieter Wuille, only supports x1, x5, x9, and xd
        vSeeds.emplace_back("dnsseed.bluematt.me."); // Matt Corallo, only supports x9
        vSeeds.emplace_back("seed.aixcoin.jonasschnelli.ch."); // Jonas Schnelli, only supports x1, x5, x9, and xd
        vSeeds.emplace_back("seed.AIX.petertodd.net."); // Peter Todd, only supports x1, x5, x9, and xd
...
public:
    CTestNetParams() {
        ...
        vSeeds.emplace_back("seed.tAIX.petertodd.net.");
...
public:
    CTestNet4Params() {
        ...
        vSeeds.clear();
        // nodes with support for servicebits filtering should be at the top
        vSeeds.emplace_back("seed.testnet4.aixcoin.sprovoost.nl."); // Sjors Provoost
...
public:
    explicit SigNetParams(const SigNetOptions& options)
    {
        ...
        if (!options.challenge) {
            ...
            vSeeds.emplace_back("seed.signet.aixcoin.sprovoost.nl.");
...
public:
    explicit CRegTestParams(const RegTestOptions& opts)
    {
        ...
        m_assumeutxo_data = {
            ...
            {
                // For use by test/functional/feature_assumeutxo.py and test/functional/tool_aixcoin_chainstate.py
                .height = 299,
