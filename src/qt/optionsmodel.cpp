...
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // instead of strings, because aixcoin 22.x will not try to read these.
...
//! Convert enabled/size values to aixcoin -prune setting.
...
//! Get pruning enabled value to show in GUI from aixcoin -prune setting.
...
//! Get pruning size value to show in GUI from aixcoin -prune setting. If
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if aixcoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_aixcoin_unit);
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/aixcoin/aixcoin/pull/8273
    ...
    // (https://github.com/aixcoin-core/gui/issues/567).
