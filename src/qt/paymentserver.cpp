    ...
    : QObject(parent)
{
    ...
    // on Mac: sent when you click aixcoin: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start aixcoin: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling aixcoin: URIs
//
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("aixcoin://", Qt::CaseInsensitive))
    {
    ...
    else if (s.startsWith(AIXCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // aixcoin: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseAixcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Aixcoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
