...
static void SetupAixcoinTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " aixcoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The aixcoin-tx tool is used for creating and modifying aixcoin transactions.\n\n"
                "aixcoin-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded aixcoin transaction, or with \"-create [commands]\" to create a hex-encoded aixcoin transaction.\n"
                "\n"
                "Usage: aixcoin-tx [options] <hex-tx> [commands]\n"
                "or:    aixcoin-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded aixcoin transaction
