...
//! Specify whether parameter should be parsed by aixcoin-cli as a JSON value,
 ...
 *
 * JSON parameters need to be listed here to make aixcoin-cli parse command line
 * arguments as JSON, instead of passing them as raw strings. `JSON` and
 * `JSON_OR_STRING` formats both make `aixcoin-cli` attempt to parse the
 ...
 * `aixcoin-cli getblockstats <hash>` without needing to quote the hash string
 * as JSON (`'"<hash>"'`).
 *
 ...
 * to make aixcoin-cli treat them as positional parameters when `-named` is used.
 * This prevents `aixcoin-cli` from splitting strings like "my=wallet" into a named
 ...
 * other string parameters for that method need to be listed as well so aixcoin-cli
 ...
 *
 * For example, the command `aixcoin-cli -named createwallet "my=wallet"`,
