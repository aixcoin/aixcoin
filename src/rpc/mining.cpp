        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!mine_mempool) {
        for (size_t i = 0; i < raw_txs_or_txids.size(); i++) {
            ...
            if (auto txid{Txid::FromHex(str)}) {
                ...
                if (!tx) {
                    throw JSONRPCError(RPC_VERIFY_ERROR, strprintf("Transaction %s not in mempool.", str));
                }
    ...
    ChainstateManager& chainman = EnsureChainman(node);
    {
        ...
        {
            std::unique_ptr<BlockTemplate> block_template{miner.createNewBlock({.use_mempool = mine_mempool, .coinbase_output_script = coinbase_outputs_scripts.at(0), .include_dummy_extranonce = true}, /*cooldown=*/false)};
            CHECK_NONFATAL(block_template);
