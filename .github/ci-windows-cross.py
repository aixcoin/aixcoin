...
def print_version():
    aixcoind = Path.cwd() / "bin" / "aixcoind.exe"
    run([str(aixcoind), "-version"])


def check_manifests():
    release_dir = Path.cwd() / "bin"
    manifest_path = release_dir / "aixcoind.manifest"

    cmd_aixcoind_manifest = [
        "mt.exe",
        "-nologo",
        f"-inputresource:{release_dir / 'aixcoind.exe'}",
        f"-out:{manifest_path}",
    ]
    run(cmd_aixcoind_manifest)
...
def run_functional_tests():
    ...
    test_runner_cmd = [
        ...
        *shlex.split(os.environ.get("TEST_RUNNER_EXTRA", "").strip()),
        ...
        # See https://github.com/aixcoin/aixcoin/issues/31409.
        "--exclude",
